...
void advection_step(
   ...
   const static lf::assemble::UniformFEDofHandler dofh(
       mesh_p, {{lf::base::RefEl::kPoint(), -1},
                ...
                {lf::base::RefEl::kQuad(), -1}}); // triangle mesh, 1 dof per cell
       ...
       for (const auto *edge : cell->SubEntities(1)) {
           if (bd_flags(*edge)) {
               ...
               if (corner_edges.col(0)[0] < INLET_X) { // Left Boundary
                   ...
                   // TODO 4.5 replace the negative values by dirichlet data
                   B.row(idx) *= -1;
                   mu_u[idx] = -1;
                   mu_v[idx] = -1;
               } else if (corner_edges.col(0)[1] > TOP_WALL) { // Top Boundary
                   // TODO 4.6 replace the negative values by dirichlet data
                   B.row(idx) *= -1;
                   mu_u[idx] = -1;
                   mu_v[idx] = -1;
               } else { // Airfoil boundary
                   // TODO 4.7 replace the negative values by dirichlet data
                   B.row(idx) *= -1;
                   mu_u[idx] = -1;
                   mu_v[idx] = -1;
               }
   ...
   for (const lf::mesh::Entity *cell : mesh_p->Entities(0)) {
       ...
       for (const lf::mesh::Entity *edge : cell->SubEntities(1)) {
           if (aux_obj(*edge)[0] != cell) {
               // TODO 1.1 FILL ME!
               (*neighbour_cells_p)(*cell)[counter] = ;
           } else {
               // TODO 1.1 FILL ME!
               (*neighbour_cells_p)(*cell)[counter] = ;
           }
